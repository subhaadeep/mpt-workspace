      ...
      {navItems
        ...
        .map(item => {
          ...
          return (
            ...
            >
              <Icon className={cn('shrink-0', active ? 'h-4 w-4 text-blue-400' : 'h-4 w-4')} />
  ...
  return (
    <aside className={cn(
      ...
      <div className={cn('flex h-16 items-center border-b border-white/5 px-3 gap-3 overflow-hidden', collapse && 'justify-center')}>
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-blue-700 text-white text-sm font-bold shadow-lg shadow-blue-500/20">
          M
        </div>
        {!collapse && (
      ...
      {!collapse && user && (
        <div className="border-t border-white/5 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0">
              <p className="truncate text-xs font-medium text-slate-300">{user.full_name || user.email}</p>
              <p className="truncate text-[10px] text-slate-500 uppercase tracking-wide">{user.role}</p>
            </div>
          </div>
        </div>
      )}
    ...
    <div className="fixed inset-0 z-50 md:hidden">
      ...
      <aside className="absolute left-0 top-0 h-full w-72 bg-[#0d1424] border-r border-white/5 flex flex-col shadow-2xl">
        <div className="flex h-16 items-center justify-between border-b border-white/5 px-4">
          ...
          <button onClick={closeMobileSidebar} className="rounded-lg p-2 text-slate-500 hover:bg-white/5 hover:text-slate-300">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        {user && (
          <div className="border-t border-white/5 px-4 py-4">
            <p className="text-sm font-medium text-slate-300">{user.full_name || user.email}</p>
            <p className="text-xs text-slate-500 uppercase mt-0.5">{user.role}</p>
          </div>
        )}
      </aside>
    </div>
