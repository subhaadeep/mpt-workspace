...
type CodeEntry = { id: number; language: string | null; filename: string | null; code_content: string | null; description: string | null }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {showNewVersion ? (
          <div className="flex items-center gap-2">
            <input autoFocus placeholder="e.g. v2.0" value={newVersionName}
      ...
      <div className="space-y-3">
        {gaRuns.map(run => (
          <div key={run.id} className="rounded-2xl border border-white/5 bg-white/2 p-4">
            ...
            {run.parameter_sets && (
              <details className="mt-2"><summary className="text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-300">Parameter Sets</summary>
            ...
            {run.best_chromosomes && (
              <details className="mt-2"><summary className="text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-300">Best Chromosomes / Params</summary>
            ...
            {run.optimization_results && (
              <details className="mt-2"><summary className="text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-300">Optimization Results</summary>
  ...
  return (
    <button onClick={copy} title="Copy code"
      className={`flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium transition-all ${
        copied ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-white/8 text-slate-400 hover:bg-white/12 hover:text-slate-200 border border-white/10'
      }`}>
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="rounded-2xl border border-white/8 bg-white/2 p-4 space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={submit} disabled={saving} className={saveBtnCls + ' disabled:opacity-60'}>
              {saving ? <Spinner size="sm" /> : null} Save Code
            </button>
            <button onClick={() => setShowForm(false)} className="rounded-xl border border-white/8 px-4 py-2.5 text-sm text-slate-400 hover:bg-white/5">Cancel</button>
          </div>
        </div>
      ...
      <div className="space-y-4">
        {sections.map((s, i) => (
          <div key={i} className="rounded-2xl border border-white/5 bg-white/2 p-4 space-y-3">
            ...
            <textarea rows={4} placeholder="Content..." value={s.content} onChange={e => update(i, 'content', e.target.value)} className={areaCls} />
          </div>
        ))}
      </div>
